...
@section('new_button')
    @can('create-purchases-payments')
        ...
        <a href="{{ route('import.create', ['group' => 'purchases', 'type' => 'payments']) }}"
           class="btn btn-white btn-sm">{{ trans('import.import') }}</a>
    @endcan
    <a href="{{ route('payments.export', request()->input()) }}"
                            ...
                            <td class="col-md-2 col-lg-3 col-xl-3 d-none d-md-block text-left">
                                ...
                                @if($item->bill)
                                    @if ($item->bill->status == 'paid')
                                        <el-tooltip
                                            content="{{ $item->bill->document_number }} / {{ trans('documents.statuses.paid') }}"
                                            effect="success"
                                            :open-delay="100"
                                            placement="top">
                                    ...
                                    @elseif ($item->bill->status == 'partial')
                                        <el-tooltip
                                            content="{{ $item->bill->document_number }} / {{ trans('documents.statuses.partial') }}"
                                            effect="info"
                                            :open-delay="100"
                                            placement="top">
                                                <span class="badge badge-dot pl-2 h-0">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <a class="dropdown-item"
                                           href="{{ route('payments.show', $item->id) }}">{{ trans('general.show') }}</a>

                                        @if (!$item->reconciled)
                                            @can('update-purchases-payments')
                                                <a class="dropdown-item"
                                                   href="{{ route('payments.edit', $item->id) }}">{{ trans('general.edit') }}</a>
                                        ...
                                        @if (empty($item->document_id))
                                            @can('create-purchases-payments')
                                                <a class="dropdown-item"
                                                   href="{{ route('payments.duplicate', $item->id) }}">{{ trans('general.duplicate') }}</a>
                                                <div class="dropdown-divider"></div>
                                            @endcan
                                        @endif
