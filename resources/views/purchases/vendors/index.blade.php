...
@section('new_button')
    @can('create-purchases-vendors')
        ...
        <a href="{{ route('import.create', ['group' => 'purchases', 'type' => 'vendors']) }}"
           class="btn btn-white btn-sm">{{ trans('import.import') }}</a>
    @endcan
    <a href="{{ route('vendors.export', request()->input()) }}"
            ...
            <div class="table-responsive">
                <table class="table table-flush table-hover">
                    ...
                    <tr class="row table-head-line">
                        ...
                        <th class="col-xs-4 col-sm-3 col-md-4 col-lg-3 col-xl-3">@sortablelink('name',
                            trans('general.name'), ['filter' => 'active, visible'], ['class' => 'col-aka', 'rel' =>
                            'nofollow'])
                        </th>
                        <th class="col-md-3 col-lg-3 col-xl-3 d-none d-md-block">@sortablelink('email',
                            trans('general.email'))
                        </th>
                        <th class="col-lg-2 col-xl-2 d-none d-lg-block text-right">@sortablelink('unpaid',
                            trans('general.unpaid'))
                        </th>
                        <th class="col-xs-4 col-sm-2 col-md-2 col-lg-2 col-xl-2 text-center">@sortablelink('enabled',
                    ...
                    @foreach($vendors as $item)
                        <tr class="row align-items-center border-top-1">
                            ...
                            <td class="col-xs-4 col-sm-3 col-md-4 col-lg-3 col-xl-3">
                                <a class="col-aka long-texts d-block"
                                   href="{{ route('vendors.show', $item->id) }}">{{ $item->name }}</a>
                            </td>
                                ...
                                @else
                                    @if ($item->enabled)
                                        <badge rounded type="success"
                                               class="mw-60 d-inline-block">{{ trans('general.yes') }}</badge>
                                    @else
                                        <badge rounded type="danger"
                                               class="mw-60 d-inline-block">{{ trans('general.no') }}</badge>
                                    @endif
                                @endif
                            </td>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <a class="dropdown-item"
                                           href="{{ route('vendors.show', $item->id) }}">{{ trans('general.show') }}</a>

                                        @can('update-purchases-vendors')
                                            <a class="dropdown-item"
                                               href="{{ route('vendors.edit', $item->id) }}">{{ trans('general.edit') }}</a>
                                        @endcan

                                        @can('create-purchases-vendors')
                                            <div class="dropdown-divider"></div>
                                            <a class="dropdown-item"
                                               href="{{ route('vendors.duplicate', $item->id) }}">{{ trans('general.duplicate') }}</a>
                                        @endcan
