@extends('layout.main') @section('content')
    <section class="forms">
        ...
        <ul class="nav nav-tabs ml-4 mt-3" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" href="#warehouse-sale" role="tab"
                   data-toggle="tab">{{trans('file.Sale')}}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#warehouse-purchase" role="tab"
                   data-toggle="tab">{{trans('file.Purchase')}}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#warehouse-quotation" role="tab"
                        ...
                        @foreach($lims_quotation_data as $key=>$quotation)
                            <tr>
                                ...
                                <td>{{ date($general_setting->date_format, strtotime($quotation->created_at->toDateString())) }}
                                    <br>{{$quotation->created_at->toTimeString()}}</td>
                                ...
                                @elseif($quotation->quotation_status == 2)
                                    <td>
                                        <div class="badge badge-success">{{trans('file.Sent')}}</div>
                                    </td>
                                @endif
                            </tr>
                        @endforeach
                        ...
                        @foreach($lims_return_data as $key=>$return)
                            <tr>
                                ...
                                <td>{{ date($general_setting->date_format, strtotime($return->created_at->toDateString())) }}
                                    <br>{{ $return->created_at->toTimeString()}}</td>
                        ...
                        @foreach($lims_expense_data as $key=>$expense)
                            <tr>
                                ...
                                <td>{{ date($general_setting->date_format, strtotime($expense->created_at->toDateString())) }}
                                    <br>{{ $expense->created_at->toTimeString() }}</td>
