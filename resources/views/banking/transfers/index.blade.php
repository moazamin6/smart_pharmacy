...
@section('new_button')
    ...
    @endcan
    <a href="{{ route('import.create', ['banking', 'transfers']) }}"
       class="btn btn-white btn-sm">{{ trans('import.import') }}</a>
    <a href="{{ route('transfers.export', request()->input()) }}"
            ...
            <div class="table-responsive">
                <table class="table table-flush table-hover">
                    ...
                    <tr class="row table-head-line">
                        ...
                        <th class="col-md-2 d-none d-md-block">@sortablelink('expense_transaction.paid_at',
                            trans('general.date'), ['filter' => 'active, visible'], ['class' => 'col-aka', 'rel' =>
                            'nofollow'])
                        </th>
                        <th class="col-sm-2 col-md-3 d-none d-sm-block">@sortablelink('expense_transaction.name',
                            trans('transfers.from_account'))
                        </th>
                        <th class="col-xs-4 col-sm-4 col-md-2">@sortablelink('income_transaction.name',
                            trans('transfers.to_account'))
                        </th>
                        <th class="col-xs-4 col-sm-2 col-md-2 text-right">@sortablelink('expense_transaction.amount',
                    ...
                    @foreach($transfers as $item)
                        ...
                        <tr class="row align-items-center border-top-1">
                            ...
                            <td class="col-md-2 d-none d-md-block"><a class="col-aka"
                                                                      href="{{ route('transfers.show', $item->id) }}">@date($item->expense_transaction->paid_at)</a>
                            </td>
                            ...
                            <td class="col-xs-4 col-sm-2 col-md-2 text-right long-texts">
                                @money($item->expense_transaction->amount, $item->expense_transaction->currency_code,
                                true)
                            </td>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <a class="dropdown-item"
                                           href="{{ route('transfers.show', $item->id) }}">{{ trans('general.show') }}</a>
                                        @can('update-banking-transfers')
                                            <div class="dropdown-divider"></div>
                                            <a class="dropdown-item"
                                               href="{{ route('transfers.edit', $item->id) }}">{{ trans('general.edit') }}</a>
                                        @endcan
