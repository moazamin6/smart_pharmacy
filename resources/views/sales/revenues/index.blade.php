...
@section('new_button')
    @can('create-sales-revenues')
        ...
        <a href="{{ route('import.create', ['group' => 'sales', 'type' => 'revenues']) }}"
           class="btn btn-white btn-sm">{{ trans('import.import') }}</a>
    @endcan
    <a href="{{ route('revenues.export', request()->input()) }}"
            ...
            <div class="table-responsive">
                <table class="table table-flush table-hover">
                    ...
                    <tr class="row table-head-line">
                        ...
                        <th class="col-xs-4 col-sm-4 col-md-3 col-lg-1 col-xl-1">@sortablelink('paid_at',
                            trans('general.date'), ['filter' => 'active, visible'], ['class' => 'col-aka', 'rel' =>
                            'nofollow'])
                        </th>
                        <th class="col-xs-4 col-sm-4 col-md-3 col-lg-2 col-xl-2 text-right">@sortablelink('amount',
                            trans('general.amount'))
                        </th>
                        <th class="col-md-2 col-lg-3 col-xl-3 d-none d-md-block text-left">@sortablelink('contact.name',
                            trans_choice('general.customers', 1))
                        </th>
                        <th class="col-lg-2 col-xl-2 d-none d-lg-block text-left">@sortablelink('category.name',
                            trans_choice('general.categories', 1))
                        </th>
                        <th class="col-lg-2 col-xl-2 d-none d-lg-block text-left">@sortablelink('account.name',
                            trans_choice('general.accounts', 1))
                        </th>
                        <th class="col-xs-4 col-sm-2 col-md-2 col-lg-1 col-xl-1 text-center">
                            <a>{{ trans('general.actions') }}</a></th>
                    </tr>
                    </thead>
                    ...
                    @foreach($revenues as $item)
                        <tr class="row align-items-center border-top-1">
                            ...
                            <td class="col-xs-4 col-sm-4 col-md-3 col-lg-2 col-xl-2 text-right">@money($item->amount,
                            ...
                            <td class="col-md-2 col-lg-3 col-xl-3 d-none d-md-block text-left">
                                ...
                                @if ($item->invoice)
                                    @if ($item->invoice->status == 'paid')
                                        <el-tooltip
                                            content="{{ $item->invoice->document_number }} / {{ trans('documents.statuses.paid') }}"
                                            effect="success"
                                            :open-delay="100"
                                            placement="top">
                                    ...
                                    @elseif ($item->invoice->status == 'partial')
                                        <el-tooltip
                                            content="{{ $item->invoice->document_number }} / {{ trans('documents.statuses.partial') }}"
                                            effect="info"
                                            :open-delay="100"
                                            placement="top">
                                                <span class="badge badge-dot pl-2 h-0">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        <a class="dropdown-item"
                                           href="{{ route('revenues.show', $item->id) }}">{{ trans('general.show') }}</a>

                                        @if (!$item->reconciled)
                                            @can('update-sales-revenues')
                                                <a class="dropdown-item"
                                                   href="{{ route('revenues.edit', $item->id) }}">{{ trans('general.edit') }}</a>
                                        ...
                                        @if (empty($item->document_id))
                                            @can('create-sales-revenues')
                                                <a class="dropdown-item"
                                                   href="{{ route('revenues.duplicate', $item->id) }}">{{ trans('general.duplicate') }}</a>
                                                <div class="dropdown-divider"></div>
                                            @endcan
                                        @endif
