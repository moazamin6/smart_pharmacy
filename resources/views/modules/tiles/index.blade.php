        ...
        @if ($modules && !empty($modules->data))
            ...
            <div class="col-md-6 text-left">
                @if ($modules->current_page > 1)
                    <a href="{{ url(request()->path()) }}?page={{ $modules->current_page - 1 }}"
                       class="btn btn-white btn-sm">{!! trans('pagination.previous') !!}</a>
                @endif
            </div>
            ...
            <div class="col-md-6 text-right">
                @if ($modules->current_page < $modules->last_page)
                    <a href="{{ url(request()->path()) }}?page={{ $modules->current_page + 1 }}"
                       class="btn btn-white btn-sm">{!! trans('pagination.next') !!}</a>
                @endif
            </div>
        @else
            <div class="col-md-12">
