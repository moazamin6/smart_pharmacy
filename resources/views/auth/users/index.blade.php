        ...
        <div class="table-responsive">
            <table class="table table-flush table-hover">
                ...
                <tr class="row table-head-line">
                    ...
                    <th class="col-xs-4 col-sm-3 col-md-2 col-lg-3">@sortablelink('name', trans('general.name'),
                        ['filter' => 'active, visible'], ['class' => 'col-aka', 'rel' => 'nofollow'])
                    </th>
                    <th class="col-sm-2 col-md-2 col-lg-3 d-none d-sm-block long-texts">@sortablelink('email',
                        trans('general.email'))
                    </th>
                            ...
                            <a class="col-aka" href="{{ route('users.edit', $item->id) }}">
                                @if (setting('default.use_gravatar', '0') == '1')
                                    <img src="{{ $item->picture }}" alt="{{ $item->name }}"
                                         class="rounded-circle user-img p-1 mr-3 d-none d-md-inline"
                                         title="{{ $item->name }}">
                                ...
                                @else
                                    <img src="{{ asset('public/img/user.svg') }}"
                                         class="user-img p-1 mr-3 d-none d-md-inline" alt="{{ $item->name }}"/>
                                @endif
                                {{ $item->name }}
                            </a>
                        </td>
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                    <a class="dropdown-item"
                                       href="{{ route('users.edit', $item->id) }}">{{ trans('general.edit') }}</a>
                                    @if (user()->id != $item->id)
