...
@section('content')
    ...
    <div id="role-permissions">
        ...
        <span class="btn btn-outline-primary btn-sm"
              @click="permissionUnselectAll">{{trans('general.unselect_all')}}</span>
        ...
        <div class="nav-wrapper">
            <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                @foreach($actions as $action)
                    ...
                    <li class="nav-item">
                        <a class="nav-link mb-sm-3 mb-md-0 {{ $active_action_tab }}" id="tabs-icons-text-1-tab"
                           data-toggle="tab" href="#tab-{{ $action }}" role="tab" aria-controls="tabs-icons-text-1"
            ...
            <div class="card-body">
                <div class="tab-content">
                    @foreach($permissions as $action => $action_permissions)
                        @php $active_action_tab = ($action == 'read') ? 'active' : ''; @endphp
                        <div class="tab-pane fade show {{ $active_action_tab }}" id="tab-{{ $action }}"
                             ref="tab-{{ $action }}" role="tabpanel">
                            <span class="btn btn-primary btn-sm"
                                  @click="select('{{ $action }}')">{{ trans('general.select_all') }}</span>
                            <span class="btn btn-primary btn-sm"
                                  @click="unselect('{{ $action }}')">{{ trans('general.unselect_all') }}</span>
